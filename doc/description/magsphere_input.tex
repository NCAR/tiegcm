...
Bz, IMF By, and Vsw, but \code{CPCP} is relatively insensitive to Nsw.
For example, \code{CPCP} changes from 158 kV to 155 kV for Vsw=450 km,
$By=0$ nT, and $Bz=-15$ nT while Nsw increases from $1 cm^{-3}$ and $16 cm^{-3}$
...
\code{theta0} (= bndyfitr from the \code{subroutine setboundary}), \code{offc} (= $4.2^o$), and \code{dskofc} (= $0.0^o$),
